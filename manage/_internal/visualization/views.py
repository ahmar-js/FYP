...
def get_model_results(request):
    ...
    if request.method == 'POST':
        ...
        if selected_df_url:
            data = pd.read_csv('../Health-Informatics-Dashboard'+selected_df_url)
...
def get_prophet_results(request):
    if request.method == 'POST':
        ...
        if forecast_url:
            data = pd.read_csv('../Health-Informatics-Dashboard'+forecast_url)
...
def get_arima_results(request):
    if request.method == 'POST':
        ...
        if forecast_arima_url:
           data = pd.read_csv('../Health-Informatics-Dashboard'+forecast_arima_url)
           forecast = pd.DataFrame(data)
           print("arima here")
        json_response_arima = {
            'arimaplot':fig_arima,
...
def retrieve_column_names(request):
    ...
    if column_record is not None:
        ...
        if selected_geodataset is not None:
            ...
            if selected_gdf_url:
                data = pd.read_csv('../Health-Informatics-Dashboard' + selected_gdf_url)
